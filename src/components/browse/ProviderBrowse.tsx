  ...
  return (
    <section className={cn("py-2", className)}>
      <div className={PAGE_X}>
        <h2 className="mb-3 text-base font-bold text-white sm:text-lg">Browse by Provider</h2>
        <div className="scrollbar-hide flex gap-3 overflow-x-auto pb-1 sm:gap-3.5">
          {providers.map((provider) => (
            <Link
              key={provider.id}
              href={`/provider/${provider.slug}`}
              className="group flex w-[62px] shrink-0 flex-col items-center gap-1.5 sm:w-[68px]"
            >
